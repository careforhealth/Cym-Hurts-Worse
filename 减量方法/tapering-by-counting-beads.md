...
本页面专门介绍**数微丸法（COUNTING）**。本文将原文所称的 beads 统一译为“微丸”。

本网站所记录的减量建议来自抗抑郁药领域的专家，包括 Mark Horowitz 博士、Anders Sørensen 博士、Adele Framer、Peter Breggin 博士以及 Heather Ashton 博士（希瑟·阿什顿）。这些专家推荐一种**缓慢、平稳、有计划的精神类药物减量方案**，在整个减量过程中，每次减小的剂量越来越小。在科学界，这种减量方式被称为双曲线型（hyperbolic）、指数型（exponential）或对数型（logarithmic）减量。
...
**分别清点 3 粒胶囊中的微丸数**。如果你使用的是我们的计算器，只需输入 3 个数值，平均值会**自动**得出。
