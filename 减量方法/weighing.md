...
title: "称重法减量"
category: 减量方法
original_url: https://healthwithoutantidepressants.com/weighing/
---
...
由于药物微丸（beads）非常细小，当剂量降到约 20 mg 左右时，电子秤已无法分辨出那么小的重量变化。此时，应切换至**计数法**（Counting Method）：[https://healthwithoutantidepressants.com/counting/](https://healthwithoutantidepressants.com/counting/)。

## 引言（INTRODUCTION）

本网站运营者多年来同时管理着一个 Facebook 群组（目前已有 45,000+ 成员），并在此过程中记录总结出一套缓慢停药的方法。我们称之为 **GRADUAL TAPER（渐进式减量法）**。制作低剂量胶囊有多种方法，本页面专门介绍使用小型珠宝秤**称重法（WEIGHT method）**——通过称量服用的微丸重量来控制剂量，而不是逐一计数。

本方法的目标是逐步减少药物剂量，使减量过程（reduction）中的不适症状降至最低甚至完全消除，从而减轻停药带来的副作用。

本网站的减量建议参考了多位抗抑郁药领域的专家，包括 Mark Horowitz 博士、Anders Sorensen 博士、Adele Framer、Peter Breggin 博士以及 Heather Ashton 博士，他们均推荐**缓慢、稳定、有计划地从精神活性药物（psychoactive drugs）中减量**，并且在减量过程中每一步的降幅越来越小。在科学界，这种减量方式被称为"双曲线型"（hyperbolic）减量。
...
所有减量方法的核心思路都是：**极其精确地测量剂量，然后服用经过称重（或计数）的剂量**。由于每粒胶囊中所含的微丸数量并不相同，**你不能只是简单地从胶囊中移除若干微丸**，那样你无法得到准确的剂量！
...
- **纸质餐盘**和**剪刀**，用来制作倒回微丸用的倾倒托盘
- **锡纸**（铝箔），用来制作胶囊支架，方便将微丸倒入
...
对剩下**两粒**（原文为"next three of your capsules"——但前文取"one of your three capsules"后只剩两粒，原文此处用词"three"前后不一致；按实际剩余数取"两粒"，并标注原文疑误）胶囊重复此步骤，将每次的重量记录到计算器中。
...
**计数法**（Counting beads）也是一种常用的方法。这种方法需要逐一清点数百颗细小且带有静电的微丸，许多人觉得极具挑战性。
...
在极低剂量下，微丸数量可能只剩 50 颗左右，此时电子秤的精度已不足以准确称量。在这种情况下，你需要开始**逐颗计数**。由于此时微丸总数已经很少，计数任务并不会太繁重。
...
这也解释了为什么在减量末期，即便只是几毫克的变化、或只是减少一两颗微丸，**都会对你的戒断症状产生显著影响**。
